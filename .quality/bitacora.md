...
## Entrada #046 — Scan Sonar

| Campo | Valor |
|-------|-------|
| **Fecha y hora de ejecución** | 2026-07-07 23:43:26 UTC-06:00 |
| **Inicio** | 2026-07-07 23:43:26 UTC-06:00 |
| **Rama** | `fix/download-ticket` |
| **Proyecto** | Eventflow Asistido AI (`eventflow-asistido-ai`) |
| **Status del scan** | **ERROR** |
| **Exit code scanner** | 1 |
...
## Entrada #047 — Scan Sonar

| Campo | Valor |
|-------|-------|
| **Fecha y hora de ejecución** | 2026-07-07 23:46:00 UTC-06:00 |
| **Inicio** | 2026-07-07 23:45:26 UTC-06:00 |
| **Rama** | `fix/download-ticket` |
| **Proyecto** | Eventflow Asistido AI (`eventflow-asistido-ai`) |
| **Status del scan** | **ERROR** |
| **Exit code scanner** | 1 |
...
**Archivos analizados:** 157 · **Sin hallazgos:** 156 · **Con hallazgos:** 1 · **Issues abiertos:** 2

**Por nivel:** Menor: 2

**Por tipo:** Code smell: 2

#### Hallazgos abiertos

| Ruta | Nivel | Tipo | Status | Línea | Regla | Mensaje |
|------|-------|------|--------|-------|-------|---------|
| `src/app/features/tickets/digital-ticket-download.util.ts` | Menor | Code smell | Abierto | 100 | `typescript:S7786` | `new Error()` is too unspecific for a type check. Use `new TypeError()` instead. |
| `src/app/features/tickets/digital-ticket-download.util.ts` | Menor | Code smell | Abierto | 109 | `typescript:S4325` | This assertion is unnecessary since the receiver accepts the original type of the expression. |
...
| `src/app/features/session/session-page.ts` | — | — | ✅ OK | 66 | 0 | 0 | 0 | 100.0 | 0 |
| `src/app/features/tickets/digital-ticket-download.util.ts` | Menor | Code smell | ⚠️ Con hallazgos | 184 | 0 | 0 | 2 | 100.0 | 2 |
...
La cobertura (94.6 %) alcanza el umbral del 80 % definido en `angular.json`, lo que indica tests suficientes para el código analizado. 2 code smell(s): deuda de mantenibilidad acumulada; no bloquean por sí solos pero elevan el costo de cambio. El Quality Gate falló por: new_violations. Ajusta código, tests o umbrales en Sonar según corresponda. 2 hallazgo(s) abierto(s) en 1 archivo(s); revisa la tabla de detalle en esta entrada.

---


## Entrada #048 — Scan Sonar

| Campo | Valor |
|-------|-------|
| **Fecha y hora de ejecución** | 2026-07-08 00:15:19 UTC-06:00 |
| **Inicio** | 2026-07-08 00:14:50 UTC-06:00 |
| **Rama** | `feat/create-event` |
| **Proyecto** | Eventflow Asistido AI (`eventflow-asistido-ai`) |
| **Status del scan** | **ERROR** |
| **Exit code scanner** | 1 |
...
Análisis procesado en SonarQube; exit code 1 porque el Quality Gate no pasó (`sonar.qualitygate.wait=true`). Quality Gate **ERROR** — condiciones incumplidas: new_coverage: 55.8 (umbral: 80).
...
**Archivos analizados:** 159 · **Sin hallazgos:** 158 · **Con hallazgos:** 1 · **Issues abiertos:** 2
...
La cobertura (93.9 %) alcanza el umbral del 80 % definido en `angular.json`, lo que indica tests suficientes para el código analizado. 2 code smell(s): deuda de mantenibilidad acumulada; no bloquean por sí solos pero elevan el costo de cambio. El Quality Gate falló por: new_coverage. Ajusta código, tests o umbrales en Sonar según corresponda. 2 hallazgo(s) abierto(s) en 1 archivo(s); revisa la tabla de detalle en esta entrada.
